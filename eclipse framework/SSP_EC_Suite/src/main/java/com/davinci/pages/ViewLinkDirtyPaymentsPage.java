...
public class ViewLinkDirtyPaymentsPage  extends LoadableComponent<ViewLinkDirtyPaymentsPage> {
	...
	@FindBy(xpath ="/html/body/app-component/app-shell/section/div/div/div/credit-module/div/view-link-dirty-payment/div/form/tool-bar/div/div/h3")
	WebElement viewlinkdirtypayments;
	...
	@FindBy(xpath="/html/body/app-component/app-shell/section/div/div/div/credit-module/div/view-link-dirty-payment/div/form/tool-bar/div/div/div/button/span")
	WebElement closeButton221;
	
	
	public ViewLinkDirtyPaymentsPage(WebDriver driver, ExtentTest report) {
		this.driver = driver;
		this.extentedReport = report;
		PageFactory.initElements(driver, this);
		Log.message("Navigated to View Link Dirty Payments Page", driver, extentedReport, true);
	}
	...
	protected void isLoaded() {
		...
		if (isPageLoaded && !driver.getTitle().contains("Close")) {
			Log.fail("View Link Dirty Payments Page did not open up", driver, extentedReport);
		}
	...
	protected void load() {
		...
		isPageLoaded = true;
		WaitUtils.waitForElement(driver, viewlinkdirtypayments);
	}
	...
	public boolean verifysearchCreditPage() throws Exception {
		...
		if (!WaitUtils.waitForElement(driver, menuText) && !WaitUtils.waitForElement(driver, viewlinkdirtypayments))
