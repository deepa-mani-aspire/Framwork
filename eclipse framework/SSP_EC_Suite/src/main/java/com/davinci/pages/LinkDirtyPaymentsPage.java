...
public class LinkDirtyPaymentsPage  extends LoadableComponent<LinkDirtyPaymentsPage> {
	...
	@FindBy(xpath ="//*[@id=navbar]/ul[1]/li[1]/div/ul/li[3]/a")
	WebElement linkdirtypayments;
	...
	@FindBy(xpath="/html/body/app-component/app-shell/section/div/div/div/credit-module/div/link-dirty-payments/div[2]/form/tool-bar/div/div/div/button[2]/span")
	WebElement closeButton221;
	
	
	public LinkDirtyPaymentsPage(WebDriver driver, ExtentTest report) {
		this.driver = driver;
		this.extentedReport = report;
		PageFactory.initElements(driver, this);
		Log.message("Navigated to Link Dirty Payments Page", driver, extentedReport, true);
	}
	...
	protected void isLoaded() {
		...
		if (isPageLoaded && !driver.getTitle().contains("Close")) {
			Log.fail("Link Dirty Payments Page did not open up", driver, extentedReport);
		}
	...
	protected void load() {
		...
		isPageLoaded = true;
		WaitUtils.waitForElement(driver, linkdirtypayments);
	}
	...
	public boolean verifysearchCreditPage() throws Exception {
		...
		if (!WaitUtils.waitForElement(driver, menuText) && !WaitUtils.waitForElement(driver, linkdirtypayments))
